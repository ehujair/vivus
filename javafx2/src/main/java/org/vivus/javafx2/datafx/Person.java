...
public class Person {
	...
	static {
		testList = FXCollections.observableArrayList(new Person("Jenny", "Bond", false, 23.64),
				new Person("Billy", "James", true, -12.11),
				new Person("Timmy", "Gordon", false, 45), new Person("Aiden", "Simpson", false, 0),
				new Person("Jacob", "Grant", true, -92.21), new Person("Jackson", "Matthews",
						false, 0), new Person("Ethan", "Beck", false, 48.12), new Person("Sophia",
						"Potts", true, 38.22), new Person("Isabella", "Bair", false, 823.43),
				new Person("Olivia", "Fowler", false, -201.23), new Person("Jayden", "Walker",
						true, 49.54), new Person("Emma", "Wong", false, -3.49), new Person("Chloe",
						"Samuelsson", false, 0.76), new Person("Logan", "Grieve", true, 49.22),
				new Person("Caden", "Sato", false, 90.56),
				new Person("Lilly", "Chin", false, -0.06), new Person("Madison", "Barbashov", true,
						89.76), new Person("Ryan", "Beatty", false, 123.50), new Person("Hailey",
						"Giles", false, 90.56), new Person("Molly", "Vos", true, -87.12),
				new Person("Nolan", "Antonio", false, 992.12), new Person("Bryce", "Marinacci",
						false, 1832.29), new Person("Maria", "Mayhew", true, -782.12), new Person(
						"Lauren", "Holt", false, 291.21));
	}
	...
	public Person() {
		...
		this.employeeTypeProperty = new SimpleObjectProperty<EmployeeType>(this, "employeeType",
	...
	private final ObservableList<Double> historicEarnings = FXCollections
			.observableArrayList(new LinkedList<Double>());
	private final ObservableList<Double> unmodifiableHistoricEarnings = FXCollections
			.unmodifiableObservableList(historicEarnings);

	@ReturnsUnmodifiableCollection
	public final ObservableList<Double> getHistoricEarnings() {
		return unmodifiableHistoricEarnings;
	}

	private final DoubleProperty totalEarningsProperty;

	public final DoubleProperty totalEarningsProperty() {
		return totalEarningsProperty;
	}
	...
	private void setTotalEarnings(double earnings) {
		totalEarningsProperty.set(earnings);
	}

	private final ObjectProperty<EmployeeType> employeeTypeProperty;

	public final ObjectProperty<EmployeeType> employeeTypeProperty() {
	...
	public final void setEmployeeType(EmployeeType type) {
		employeeTypeProperty.set(type);
	}
	...
	private void setTotalSales(double earnings) {
		totalSalesProperty.set(earnings);
	}
	...
	public Person(String firstName, String lastName, boolean telecommuter, double balance) {
		...
		if (DEBUG) {
			this.firstNameProperty.addListener(new ChangeListener<String>() {
				@Override
				public void changed(javafx.beans.value.ObservableValue<? extends String> arg0,
						String ov, String nv) {
			...
			this.telecommuterProperty.addListener(new ChangeListener<Boolean>() {
				@Override
				public void changed(javafx.beans.value.ObservableValue<? extends Boolean> arg0,
			...
			public void run() {
				while (liveUpdate) {
					for (Person p : getTestList()) {
						...
						if (updateEarnings) {
							final double earnings = p.getEarnings()
									+ (Math.random() * 1 * (r.nextBoolean() ? 1 : -1));
