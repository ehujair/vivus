package org.vivus.javafx2.chart;
...
public class DynamicallyColoredBarChartWithLabel extends Application {
	...
	public void start(Stage stage) {
		...
		chartWithLegend.getStylesheets().add(
				getClass().getResource("colored-chart.css").toExternalForm());
	...
	private void displayLabelForData(XYChart.Data<String, Number> data) {
		...
		node.parentProperty().addListener(new ChangeListener<Parent>() {
			@Override
			public void changed(ObservableValue<? extends Parent> ov, Parent oldParent,
					Parent parent) {
