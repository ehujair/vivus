...
public class PrintTest1 implements Printable {
	...
	public int print(Graphics gra, PageFormat pf, int pageIndex) throws PrinterException {
		...
		case 0:
			...
			float[] dash1 = { 4.0f };
			g2.setStroke(new BasicStroke(0.5f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 4.0f,
					dash1, 0.0f));
			...
			Image src = Toolkit.getDefaultToolkit().getImage("70822790.jpg");
			...
			g2.drawLine((int) x, (int) (y + 1 * heigth + img_Height + 10), (int) x + 200, (int) (y
					+ 1 * heigth + img_Height + 10));
			...
			// LineBreakMeasurer lineBreaker = new
			// LineBreakMeasurer(text.getIterator(), frc);
			// //每行字符显示长度(点)
			// double width = pf.getImageableWidth();
			// //首字符的基线位于用户空间中的 (x, y) 位置处
			// Point2D.Double pen = new Point2D.Double (100, y+3*heigth);
			// while ( (layout = lineBreaker.nextLayout( (float) width)) !=
			// null){
